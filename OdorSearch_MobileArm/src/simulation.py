...
参考 OdorSim 的 `odor_sim.runtime.session.OdorCosimSession`：
- 组合 Environment + Robot + Agent + SensorArray；
- 提供 `reset()` / `step()` / `run()` 统一接口；
- 每步记录状态、传感器读数、搜索阶段、是否碰撞。
"""
...
class OdorSearchSession:
    """气味源搜索仿真会话。

    Args:
        robot_config: 机器人配置字典或路径。
        env_config: 环境配置字典或路径。
        seed: 随机种子。
    """
    ...
    def __init__(
        ...
        self.agent = OdorSearchAgent(self.robot, self.env, vision=self.vision)
    ...
    def step(self) -> tuple[dict[str, Any], bool, dict[str, Any]]:
        ...
        record = {
            ...
            "collision": collision,
            "info": {k: v for k, v in info.items() if k not in ("target_point",)},
        }
