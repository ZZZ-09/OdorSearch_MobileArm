...
class WarehouseVisualizer:
    ...
    def draw_static_environment(self) -> None:
        ...
        for obs in self.env.obstacles:
            if obs["type"] == "box":
                self._draw_box(obs["center"], obs["size"], color="gray", alpha=0.4)
            elif obs["type"] == "cylinder":
                self._draw_cylinder(
                    obs["center"], obs["axis"], obs["radius"], obs["length"], color="steelblue", alpha=0.4
                )
    ...
    def _draw_box(
        ...
        for e in edges:
            pts = corners[e]
            self.ax.plot(pts[:, 0], pts[:, 1], pts[:, 2], color=color, alpha=alpha)
    ...
    def _draw_cylinder(
        ...
        for z in [-length / 2.0, length / 2.0]:
            ...
            local[2, :] = z
            world = (R @ local).T + c
            self.ax.plot(world[:, 0], world[:, 1], world[:, 2], color=color, alpha=alpha)
