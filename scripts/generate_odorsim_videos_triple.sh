#!/usr/bin/env bash
# Generate triple-view OdorSim/GADEN videos for a list of seeds with obstacle 8x8 config.
# Usage (from WSL, after sourcing OdorSim/setup/activate.sh):
#   bash scripts/generate_odorsim_videos_triple.sh [seed1 seed2 ...]
set -uo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ENV_CONFIG="config/warehouse_obstacle_8x8.yaml"
OUT_DIR="$PROJECT_ROOT/outputs/odorsim_obstacle_new30_videos"
SCENARIO="$PROJECT_ROOT/odorsim_scenarios/warehouse_8x8/environment_configurations/config1"

mkdir -p "$OUT_DIR"
cd "$PROJECT_ROOT"

SEEDS="${@:-28488 50541 63219 70542 91814}"
