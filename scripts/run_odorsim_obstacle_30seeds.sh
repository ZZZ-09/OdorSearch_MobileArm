#!/usr/bin/env bash
# Run OdorSim/GADEN verification for 30 random seeds with obstacle 8x8 config.
# Usage (from WSL, after sourcing OdorSim/setup/activate.sh):
#   bash scripts/run_odorsim_obstacle_30seeds.sh
set -uo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
# 30 new seeds generated by numpy.random.default_rng(2027) in [0, 1e5),
# excluding all seeds already used in outputs/odorsim_obstacle_5seeds,
# outputs/odorsim_obstacle_30seeds and outputs/odorsim_obstacle.
...
OUT_DIR="$PROJECT_ROOT/outputs/odorsim_obstacle_new30"
