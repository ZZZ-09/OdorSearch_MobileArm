#!/usr/bin/env bash
# Generate triple-view OdorSim/GADEN videos for a list of seeds with obstacle 8x8 config (improved algorithm v2).
# Usage (from WSL, after sourcing OdorSim/setup/activate.sh):
#   bash scripts/generate_odorsim_videos_triple_v2.sh [seed1 seed2 ...]
set -uo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ENV_CONFIG="config/warehouse_obstacle_8x8.yaml"
OUT_DIR="$PROJECT_ROOT/outputs/odorsim_obstacle_new30_v2_videos"
...
SEEDS="${@:-}"
if [ -z "$SEEDS" ]; then
    echo "Usage: bash scripts/generate_odorsim_videos_triple_v2.sh seed1 seed2 seed3 seed4 seed5"
    exit 1
fi
